...
func rpcCredentials(idpAddress string) (credentials.PerRPCCredentials, error) {
	tks := new(tokens.Tokens)
	if err := tks.Read(); err != nil {
		return nil, err
	}

	if tks.Issuer != idpAddress {
		return nil, errors.New("grpc: existing tokens generated by different identity provider")
	}
